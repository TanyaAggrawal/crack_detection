...
class DroneImage:
    ...
    def __load__(self):
        ...
        try:
            ...
            print(self.image_name, 'can Not read pred, replace with zeros hi hello')
            self.raw_pred = []
    ...
    def get_subimages(self, index, size, kind=0):
        ...
        for point in self.pick_index:
            ...
            if abs(p_x - x) + abs(p_y - y) < self.close_standard:
                self.too_close = True
                break
        self.pick_index.append(
            [[x - size[0] // 2, x - size[0] // 2, x + size[0] // 2, x + size[0] // 2, x - size[0] // 2],
...
class DataSetBalanced:
    ...
    def __load_one_image__(self, name):
        result = DroneImage(name, load=True, label_appendix=self.label_appendix, pred_appendix=self.pred_appendix,
                            image_appendix=self.image_appendix)
